...
- 노드·관계 선택에 따라 필터링되는 법령·조문·직접 원문·상위 문맥 근거
...
현재 기본 설정은 엔티티 Top-8, 릴레이션 Top-20, 엔티티별 인접 관계 50, hop별 beam 160, 최종 경로 50이다. OpenSearch 후보 단계는 키워드와 벡터 결과를 RRF로 결합한다. 그래프 확장은 **릴레이션 Top-20에 포함된 관계만 따라가는 방식이 아니다.** 각 hop에서 특정 고차수 frontier가 전체 조회 한도를 독점하지 않도록 frontier별 neighbor 한도를 SQL에서 먼저 적용하고, 릴레이션 Top-20 후보는 그 한도 안에서 우선 조회한다. 후보 밖 관계도 계속 확장 대상이다. 경로 점수에는 관계 후보 관련도, 질문에 포함된 엔티티·관계명의 구체성, 근거 수, 질문 방향 단서와 분기 수 패널티를 반영한다. 상위 경로에 RDB 근거를 연결한 뒤에는 `unit_text`, `context_text`, 문서 제목과 질문의 어휘 일치도로 다시 정렬한다. 여러 문서를 포함했다는 이유만으로 가산점을 주지는 않는다.
...
웹 화면과 오프라인 `graph.html`은 저장소 전체 그래프가 아니라, 엔티티/관계 검색 후보와 상위 경로에서 나온 제한된 탐색 맥락을 함께 표시한다. 최종 선택 경로의 노드·간선은 색으로 강조하고, 선택되지 않은 후보 전용 노드·관계는 회색 점·간선으로 남긴다. 따라서 후보를 클릭해 확인할 수 있지만, 근거 조문은 최종 경로에 실제로 포함된 관계에만 표시된다.

## LLM 근거 답변

`[answer]`에서 `enabled = true`이면 최종 경로에 실제로 연결된 `graph.semantic_unit`의 직접 원문(`unit_text`)과 상위 문맥(`context_text`), 문서 제목, 조문 위치, 근거 SVO를 loopback OpenAI 호환 LLM에 전달해 답변을 생성한다. 후보 목록·AGE 전체 그래프·선택되지 않은 원문은 전달하지 않는다. 모델은 원문을 우선 근거로 사용하고 각 사실에 `[E1]` 같은 근거 키를 붙인 JSON 답변만 반환해야 하며, 존재하지 않는 키·인용 없는 응답·잘못된 JSON은 거부한다.

LLM 호출 실패, 검증 실패 또는 남은 전체 시간 예산 부족 시에는 기존 결정적 근거 답변으로 자동 fallback한다. 화면과 `query_result.json`, `search_manifest.json`에는 생성 여부, fallback 사유, 모델, 프롬프트 파일명·SHA-256, 인용 키가 기록된다.

기본 LLM은 `.env`의 `VLLM_BASE_URL`에 설정한 loopback 주소만 허용한다. 비-loopback 내부 엔드포인트는 사용자가 명시적으로 `--allow-remote-llm`을 전달했을 때만 허용한다.
