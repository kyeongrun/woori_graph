...
from ..entity_typing import ENTITY_TYPES, ENTITY_TYPE_SET, validate_typed_entity_dictionary
...
def _validate_source_records(raw_records: Sequence[Mapping[str, Any]]) -> None:
    """Require the source fields needed to reproduce and ground every relation."""
    ...
    for index, record in enumerate(raw_records, start=1):
        ...
        if missing:
            raise ValueError(
                f"raw SVO record {index} is missing source fields: {missing}"
            )
        if not isinstance(record["source_ref"], Mapping):
            raise ValueError(f"raw SVO record {index} source_ref must be an object")
        if not str(record["unit_text"]).strip():
            raise ValueError(f"raw SVO record {index} unit_text must be non-empty")
        if not str(record["unit_kind"]).strip():
            raise ValueError(f"raw SVO record {index} unit_kind must be non-empty")
...
def build_storage_load_files(
    ...
    *,
    ...
    validate_typed_entity_dictionary(entity_dictionary)
    _validate_source_records(raw_records)
    ...
    counts = {
        ...
        "semantic_units": len(records["semantic_units"]),
...
def _materialize_records(
    ...
    return {
        ...
        "semantic_units": list(bundle.semantic_units),
        "entities": entities,
    ...
    *,
    ...
    _write_csv(
        ...
        (
            ...
            for item in records["semantic_units"]
        ),
        overwrite=overwrite,
    )
    _write_csv(
...
CREATE TABLE IF NOT EXISTS graph.semantic_unit (
    semantic_unit_id uuid PRIMARY KEY,
    document_id uuid NOT NULL REFERENCES graph.document(document_id),
    source_ref jsonb NOT NULL,
    context_text text NOT NULL,
    unit_text text NOT NULL CHECK (length(btrim(unit_text)) > 0),
    unit_kind text NOT NULL CHECK (length(btrim(unit_kind)) > 0),
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now()
);
...
DO $$
BEGIN
    ALTER TABLE graph.relation_evidence
        ADD CONSTRAINT relation_evidence_semantic_unit_fk
        FOREIGN KEY (semantic_unit_id)
        REFERENCES graph.semantic_unit(semantic_unit_id)
        NOT VALID;
EXCEPTION
...
CREATE INDEX IF NOT EXISTS semantic_unit_document_idx
    ON graph.semantic_unit (document_id);
"""
...
CREATE TEMP TABLE stage_semantic_unit (
    semantic_unit_id text, document_id text, source_ref_json text,
    context_text text, unit_text text, unit_kind text
);
...
\copy stage_semantic_unit FROM '{base}/{release}/rdb/semantic_units.csv' WITH (FORMAT csv, HEADER true)
...
INSERT INTO graph.semantic_unit (
    semantic_unit_id, document_id, source_ref, context_text, unit_text, unit_kind
)
SELECT semantic_unit_id::uuid, document_id::uuid, source_ref_json::jsonb,
       context_text, unit_text, unit_kind
FROM stage_semantic_unit
...
ALTER TABLE graph.relation_evidence
    VALIDATE CONSTRAINT relation_evidence_semantic_unit_fk;
...
def _write_age_files(
    ...
    *,
    ...
    entity_types_by_id = {
        item["entity_id"]: item["entity_type"] for item in records["entities"]
    }
    vertex_files: list[dict[str, Any]] = []
    for entity_type in ENTITY_TYPES:
        typed_entities = [
            item for item in records["entities"] if item["entity_type"] == entity_type
        ]
        filename = f"vertices_{entity_type.lower()}.csv"
        _write_csv(
            age_dir / filename,
            ["id", "uuid"],
            (
                [
                    entity_numeric_ids[item["entity_id"]],
                    item["entity_id"],
                ]
        ...
        _write_csv(
            ...
            (
                [
                    ...
                    entity_types_by_id[item["source_entity_id"]],
                    entity_numeric_ids[item["target_entity_id"]],
                    entity_types_by_id[item["target_entity_id"]],
                    item["relation_id"],
                    item["source_name"],
                    item["target_name"],
                ]
    ...
    for item in vertex_files:
        label = item["label"]
        sql_lines.append(
            f"SELECT create_vlabel('{_sql_literal(age_graph_name)}', '{label}');"
        )
        if item["count"]:
            sql_lines.extend(
                [
                    ...
                    f"    $$ MATCH (n:`{label}`) SET n.id = n.uuid REMOVE n.uuid $$",
                    ") AS (result agtype);",
                ]
            )
        ...
        if item["count"]:
            sql_lines.extend(
                [
                    "SELECT * FROM cypher(",
                    f"    '{_sql_literal(age_graph_name)}',",
                    f"    $$ MATCH (n:`{label}`) REMOVE n.__id__ $$",
                    ") AS (result agtype);",
                    f'CREATE INDEX "{age_graph_name}_{label.lower()}_properties_gin"',
                    f'    ON "{graph_schema}"."{label}" USING gin (properties);',
                ]
            )
    sql_lines.append("")
    _write_text(age_dir / "load.sql", "\n".join(sql_lines), overwrite=overwrite)
    return {
        "vertex_labels": vertex_files,
        ...
        "internal_id_note": (
            ...
            "Entity type is encoded by the vertex label, not duplicated as a property."
        ),
    }
...
def _audit_cross_store_files(
    ...
    document_ids = {item["document_id"] for item in records["documents"]}
    semantic_units_by_id = {
        item["semantic_unit_id"]: item for item in records["semantic_units"]
    }
    ...
    age_vertex_uuids: set[str] = set()
    age_vertex_ids_by_type: dict[str, set[str]] = {}
    for entity_type in ENTITY_TYPES:
        path = output_dir / "age" / f"vertices_{entity_type.lower()}.csv"
        values = _csv_values(path, "uuid")
        age_vertex_ids_by_type[entity_type] = values
        age_vertex_uuids.update(values)
    ...
    return {
        "all_semantic_unit_documents_exist": all(
            item["document_id"] in document_ids
            for item in records["semantic_units"]
        ),
        "all_semantic_units_preserve_source": all(
            isinstance(item.get("source_ref"), Mapping)
            and isinstance(item.get("context_text"), str)
            and bool(str(item.get("unit_text", "")).strip())
            and bool(str(item.get("unit_kind", "")).strip())
            for item in records["semantic_units"]
        ),
        "all_evidence_semantic_units_exist": all(
            item["semantic_unit_id"] in semantic_units_by_id
            for item in records["evidence"]
        ),
        "all_evidence_source_locations_match": all(
            item["document_id"]
            == semantic_units_by_id.get(item["semantic_unit_id"], {}).get("document_id")
            and item["source_ref"]
            == semantic_units_by_id.get(item["semantic_unit_id"], {}).get("source_ref")
            for item in records["evidence"]
        ),
        "all_entity_types_present": all(item.get("entity_type") for item in records["entities"]),
        "all_entity_types_valid": all(
            item.get("entity_type") in ENTITY_TYPE_SET for item in records["entities"]
        ),
        "age_vertex_labels_match_entity_types": all(
            age_vertex_ids_by_type[entity_type]
            == {
                item["entity_id"]
                for item in records["entities"]
