...
def build_search_web_payload(
    ...
    *,
    ...
    citation_keys = {
        item.relation_mention_id: item.key
        for item in result.answer_metadata.citation_map
    }
    ...
    for edge in graph["edges"]:
        for evidence in edge["evidence"]:
            evidence_by_id.setdefault(
                ...
                {
                    **evidence,
                    "citation_key": citation_keys.get(evidence["relation_mention_id"]),
    ...
    return {
        ...
        "answer_metadata": result.answer_metadata.to_dict(),
        "graph": graph,
        "evidence": evidence,
        ...
        "search_plan": {
            ...
            "traversal": {
                ...
                "rule": (
                    "각 hop에서 frontier별 인접 관계를 공정하게 조회하고 관계 Top-K 후보는 "
                    "우선 보존합니다. 후보 밖 관계도 확장하며, 구체적인 이름 일치·관계 관련도·"
                    "분기 수·근거 수로 점수화한 뒤 원문 근거의 질문 일치도로 재정렬합니다."
                ),
        ...
        "counts": {
            ...
            "selected_nodes": graph["selected_counts"]["nodes"],
            "selected_edges": graph["selected_counts"]["edges"],
            "context_nodes": graph["context_counts"]["nodes"],
            "context_edges": graph["context_counts"]["edges"],
