...
class SearchStats:
    ...
    hop_diagnostics: tuple[HopDiagnostic, ...] = ()
    deduplicated_paths: int = 0
    evidence_path_pool: int = 0
...
class AnswerCitation:
    """A stable citation key exposed to the LLM and the search UI."""

    key: str
    relation_mention_id: str

    def to_dict(self) -> dict[str, str]:
        return {"key": self.key, "relation_mention_id": self.relation_mention_id}


@dataclass(frozen=True)
class AnswerMetadata:
    """Provenance for either an LLM answer or its deterministic fallback."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "citation_map": [item.to_dict() for item in self.citation_map],
...
class SearchResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "answer_metadata": self.answer_metadata.to_dict(),
        }
