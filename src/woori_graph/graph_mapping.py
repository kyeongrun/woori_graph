...
from .entity_typing import infer_entity_type, validate_typed_entity_dictionary
...
def map_raw_svo_to_graph(
    ...
    *,
    ...
    semantic_unit_buckets: dict[str, dict[str, Any]] = {}
    ...
    for record in raw_records:
        ...
        semantic_unit = {
            ...
            "context_text": str(record.get("context_text", "")),
            "unit_text": str(record.get("unit_text", "")),
            "unit_kind": str(record.get("unit_kind", "unknown")),
        }
        existing_unit = semantic_unit_buckets.setdefault(
            semantic_unit_id, semantic_unit
        )
        if existing_unit != semantic_unit:
            raise ValueError(
                "conflicting source data for semantic_unit_id "
                f"{semantic_unit_id!r}"
            )
        for relation in record.get("relations", []):
    ...
    return GraphLoadBundle(
        ...
        semantic_units=sorted(
            semantic_unit_buckets.values(), key=lambda item: item["semantic_unit_id"]
        ),
        entities=entities,
...
def _resolve_entity_for_load(
    ...
    if dictionary_entity is None:
        ...
        entity_type = infer_entity_type(resolved_name).entity_type
        mapping_status = (
            resolution_method
        ...
        entity_type = dictionary_entity["entity_type"]
        if resolution_method == "current_document_self_reference":
