...
def test_graph_mapping_maps_entity_alias_and_loads_unknown_name_unchanged() -> None:
    ...
    assert penalty["entity_type"] == "CONCEPT"
...
def test_graph_mapping_rejects_released_entity_without_type() -> None:
    dictionary = _entity_dictionary()
    dictionary[0].pop("entity_type")

    with pytest.raises(EntityTypeValidationError, match="classify-entity-types"):
        map_raw_svo_to_graph(
            [], dictionary, _relation_dictionary(), dictionary_version="v1"
        )
...
def test_same_raw_entity_name_has_one_id_across_documents() -> None:
    ...
    assert committee["entity_type"] == "ORGANIZATION"
...
def test_self_reference_uses_document_title_without_becoming_global_alias() -> None:
    ...
    assert law["entity_type"] == "LEGAL_INSTRUMENT"
