    ...
    *,
    ...
    def normalize_batch(batch_index: int, batch: list[dict[str, Any]]):
        ...
        response = client.complete(
            f"{CONTEXTUAL_ENTITY_NORMALIZATION_PROMPT}\n\n입력:\n"
    ...
    sanitized = sanitize_contextual_entity_mapping(list(mappings.values()))
    return sorted(
...
def audit_contextual_entity_dictionary(
    ...
    introduced_generic_names = [
        ...
        for item in mapping_records
        if item["canonical_name"] in _GENERIC_ONLY_NAMES
        and item["source_canonical_name"] != item["canonical_name"]
    ]
    checks = {
