...
class GraphRepository(Protocol):
    def expand(self, entity_ids: Sequence[str], *, max_edges: int) -> list[GraphEdge]: ...
...
class PostgresGraphRepository:
    ...
    def expand(self, entity_ids: Sequence[str], *, max_edges: int) -> list[GraphEdge]:
        ...
        sql = """
            ...
            WHERE r.source_entity_id = ANY(%s::uuid[])
               OR r.target_entity_id = ANY(%s::uuid[])
            ORDER BY r.evidence_count DESC, r.relation_id
            LIMIT %s
        """
        with self._connection.cursor() as cursor:
            cursor.execute(sql, (identifiers, identifiers, max_edges))
        ...
        sql = """
            WITH per_document AS (
                SELECT re.relation_mention_id::text, re.relation_id::text,
                       re.semantic_unit_id::text, re.document_id::text,
                       d.document_title, d.source_path, re.source_ref,
                       re.raw_subject, re.raw_predicate, re.raw_object,
                       row_number() OVER (
                           PARTITION BY re.relation_id, re.document_id
                           ORDER BY re.source_ref::text, re.relation_mention_id
                       ) AS document_rank
                FROM graph.relation_evidence AS re
                JOIN graph.document AS d USING (document_id)
    ...
    def load_all_edges_with_evidence(self) -> list[GraphEdge]:
        ...
        evidence_sql = """
            ...
            FROM graph.relation_evidence AS re
            JOIN graph.document AS d USING (document_id)
            ORDER BY re.relation_id, d.document_title, re.source_ref::text,
