...
def build_graph_payload(result: SearchResult, *, path_limit: int = 20) -> dict[str, Any]:
    ...
    for path in result.paths[:path_limit]:
        for level, (identifier, name) in enumerate(
            ...
            payload = {
                "id": identifier,
                "name": name,
                "type": candidate.entity_type if candidate else "OTHER",
                "seed": candidate is not None,
                "candidate_score": round(candidate.score, 8) if candidate else 0.0,
                "level": level,
            }
        ...
        for edge in path.edges:
            edges.setdefault(
                edge.relation_id,
                {
                    ...
                    "documents": sorted(
                        {item.document_title for item in edge.evidences}
                    ),
    ...
    return {
        ...
        "nodes": sorted(nodes.values(), key=lambda item: (item["level"], item["name"], item["id"])),
