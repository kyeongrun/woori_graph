...
이 디렉터리는 내부 다중 API 서버에 통합할 최신 소스와 동일 60개 법령 재현 자료를 담는다. 개발·검증용 `search-web` 로컬 서버는 포함하지만 운영 API 라우터는 포함하지 않으며, 내부 코드 어시스턴트는 아래 application service와 repository 포트를 내부 프로젝트 구조에 연결한다.
...
- `src/woori_graph/search/`: 질문 임베딩 → OpenSearch 후보 검색 → PostgreSQL 최대 3-hop 탐색 → 근거 답변·웹 시각화·탐색 진단
...
로컬 웹 화면은 다음 명령으로 실행한다.

```powershell
woori-graph search-web --config config/search.example.toml --port 8765 --allow-remote-embedding
```

`http://127.0.0.1:8765/`에서 질문을 입력하면 답변, 3-hop 그래프, 근거, hop별 조회·생성·beam 잔존 수와 선정된 엔티티·릴레이션 후보 목록을 확인할 수 있다.
...
- 웹 검색 화면: 데스크톱·모바일, 질문 제출, 노드 선택, 후보·hop 진단 표시 검증
