...
def test_build_storage_load_files_reuses_uuid_strings_across_stores(
    ...
    with (output / "age" / "entities.csv").open(encoding="utf-8", newline="") as handle:
        age_entity_ids = {row["uuid"] for row in csv.DictReader(handle)}
...
def test_storage_load_files_default_missing_entity_type_to_other(
    storage_workspace,
) -> None:
    raw, entities, relations = _fixture_records()
    output = storage_workspace / "release_v1"
    ...
    assert {record["entity_type"] for record in records} == {"OTHER"}
